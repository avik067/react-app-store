...
class AppStore extends Component {
  state = {tabId: tabsList[0].tabId, keyWord: ''}

  change = idPassing => {
    this.setState({tabId: idPassing})
  }

  tabOnlyList = () => {
    const {tabId} = this.state

    const newList = appsList.filter(each => each.category === tabId)

    return newList
  }

  searchOnkeyWord = newList => {
    const {keyWord} = this.state
    const afterSeachList = newList.filter(each => {
      const keyWordLower = keyWord.toLowerCase()
      const eachLower = each.appName.toLowerCase()
      return eachLower.includes(keyWordLower)
    })
    return afterSeachList
  }
  ...
  render() {
    ...
    const searchList = this.searchOnkeyWord(newList)
